...
double Value::getDoubleValue(Value *from, Interpreter *interpreter)
{
    ...
    case VALUE_TYPE_OBJECT:
    {
        if (interpreter != NULL)
        {
            Function *f = from->ovalue->getClass()->getFunctionByName("toNumber");
            if (f == NULL)
            {
                throw std::logic_error("toNumber function not available for object. Cannot get number value");
            }
